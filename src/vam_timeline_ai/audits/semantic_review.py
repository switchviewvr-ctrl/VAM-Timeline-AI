...
def export_semantic_review_010(
    ...
    use_pose_anchor_completeness: bool = False,
    use_controller_orientation_validity: bool = False,
) -> dict[str, Any]:
    if count != 10:
    ...
    elif use_cowgirl_candidate_score_v6 or use_controller_validity:
    ...
    for idx, item in enumerate(selected, start=1):
        ...
        if attempt_timeline_export:
            export_result = _attempt_timeline_export(row, data, timeline_root / row["review_id"], export_mode=export_mode)
        else:
...
def _load_data(run: Path) -> dict[str, Any]:
    return {
        ...
        "pose_anchor_completeness": {r.get("window_id"): r for r in load_jsonl(run / "audits" / "pose_anchor_completeness.jsonl") if r.get("window_id")},
...
def _select_10_v9(data: dict[str, Any]) -> list[dict[str, Any]]:
    flags = data.get("selection_flags", {})
    min_duration = float(flags.get("min_cowgirl_window_seconds") or 4.0)
    quotas = {
        "semantic_cowgirl_generation_safe": 3,
        "semantic_cowgirl_anchor_incomplete": 2,
        "semantic_cowgirl_controller_outlier": 1,
        "cowgirl_context_intro_low_motion": 1,
        "receiver_response_negative": 1,
        "unknown_or_unusable": 1,
        "export_unavailable_or_missing_anchor": 1,
    }
    pools = {key: [] for key in quotas}
    for wid, score in data["cowgirl_scores_v7"].items():
        duration = float(score.get("duration_seconds") or 0.0)
        if score.get("semantic_cowgirl_generation_safe") and duration >= min_duration:
            pools["semantic_cowgirl_generation_safe"].append(
                _candidate(
                    "semantic_cowgirl_generation_safe",
                    wid,
                    _pair_id_from_role_score(data["rider_receiver_scores"].get(wid, {})),
                    float(score.get("final_generation_candidate_score_v7") or 0.0),
                    ["semantic Cowgirl and pose-anchor complete for review/generation candidate"],
                    ["semantic_cowgirl_generation_safe", "pose_anchor_controllers_present"],
                )
            )
        if score.get("semantic_cowgirl_anchor_incomplete"):
            pools["semantic_cowgirl_anchor_incomplete"].append(
                _candidate(
                    "semantic_cowgirl_anchor_incomplete",
                    wid,
                    _pair_id_from_role_score(data["rider_receiver_scores"].get(wid, {})),
                    float(score.get("final_semantic_cowgirl_score_v7") or 0.0),
                    ["semantic Cowgirl but missing pose-critical static anchors"],
                    ["pose_anchor_incomplete", *(score.get("missing_required_anchor_controllers") or [])],
                )
            )
        if score.get("semantic_cowgirl_controller_outlier"):
            pools["semantic_cowgirl_controller_outlier"].append(
                _candidate(
                    "semantic_cowgirl_controller_outlier",
                    wid,
                    _pair_id_from_role_score(data["rider_receiver_scores"].get(wid, {})),
                    float(score.get("final_semantic_cowgirl_score_v7") or 0.0),
                    ["semantic Cowgirl but controller outlier makes generation unsafe"],
                    ["controller_outlier", "generation_pose_invalid"],
                )
            )
        if score.get("cowgirl_context_intro_low_motion"):
            pools["cowgirl_context_intro_low_motion"].append(
                _candidate(
                    "cowgirl_context_intro_low_motion",
                    wid,
                    None,
                    float(score.get("cowgirl_context_score") or 0.0),
                    ["Cowgirl context/intro or low-motion pose; not clean motion"],
                    ["cowgirl_context_intro_low_motion"],
                )
            )
        if score.get("receiver_response_negative"):
            pools["receiver_response_negative"].append(
                _candidate(
                    "receiver_response_negative",
                    wid,
                    _pair_id_from_role_score(data["rider_receiver_scores"].get(wid, {})),
                    float(score.get("receiver_body_response_score") or 0.0),
                    ["receiver/body-response negative retained for audit"],
                    ["receiver_body_response", "not_active_rider"],
                )
            )
        if score.get("unknown_or_unusable"):
            pools["unknown_or_unusable"].append(
                _candidate(
                    "unknown_or_unusable",
                    wid,
                    None,
                    max(float(score.get("final_semantic_cowgirl_score_v7") or 0.0), 0.25),
                    ["unknown/unusable or export unavailable case"],
                    ["unknown_or_unusable"],
                )
            )
        if score.get("export_unavailable_for_generation") or score.get("missing_foot_controllers") or score.get("missing_knee_controllers"):
            pools["export_unavailable_or_missing_anchor"].append(
                _candidate(
                    "export_unavailable_or_missing_anchor",
                    wid,
                    None,
                    max(float(score.get("final_semantic_cowgirl_score_v7") or 0.0), float(score.get("pose_anchor_completeness_score") or 0.0)),
                    ["export unavailable or missing required pose anchors"],
                    ["export_unavailable_or_missing_anchor"],
                )
            )
    for rows in pools.values():
...
def _select_10_v10(data: dict[str, Any]) -> list[dict[str, Any]]:
    flags = data.get("selection_flags", {})
    min_duration = float(flags.get("min_cowgirl_window_seconds") or 4.0)
    quotas = {
        "semantic_cowgirl_generation_safe": 4,
        "semantic_cowgirl_orientation_invalid": 1,
        "semantic_cowgirl_anchor_incomplete": 1,
        "standing_gesture_false_positive": 1,
        "receiver_response_negative": 1,
        "cowgirl_context_intro_low_motion": 1,
        "unknown_or_unusable": 1,
    }
    pools = {key: [] for key in quotas}
    for wid, score in data["cowgirl_scores_v8"].items():
        duration = float(score.get("duration_seconds") or 0.0)
        pair_id = _pair_id_from_role_score(data["rider_receiver_scores"].get(wid, {}))
        if score.get("semantic_cowgirl_generation_safe") and duration >= min_duration:
            pools["semantic_cowgirl_generation_safe"].append(
                _candidate(
                    "semantic_cowgirl_generation_safe",
                    wid,
                    pair_id,
                    float(score.get("final_generation_candidate_score_v8") or 0.0),
                    ["semantic Cowgirl with anchors/controller positions/orientations passing generation-safety audit"],
                    ["semantic_cowgirl_generation_safe", "controller_orientation_valid"],
                )
            )
        if score.get("semantic_cowgirl_orientation_invalid"):
            pools["semantic_cowgirl_orientation_invalid"].append(
                _candidate(
                    "semantic_cowgirl_orientation_invalid",
                    wid,
                    pair_id,
                    float(score.get("final_semantic_cowgirl_score_v8") or 0.0),
                    ["semantic Cowgirl but controller rotation/orientation appears twisted"],
                    ["controller_rotation_invalid", "controller_twist_invalid", *(score.get("twisted_controller_names") or [])],
                )
            )
        if score.get("semantic_cowgirl_anchor_incomplete"):
            pools["semantic_cowgirl_anchor_incomplete"].append(
                _candidate(
                    ...
                    pair_id,
                    float(score.get("final_semantic_cowgirl_score_v8") or 0.0),
                    ["semantic Cowgirl but missing pose-critical foot/knee anchors"],
                    ["pose_anchor_incomplete", *(score.get("missing_required_anchor_controllers") or [])],
                )
            )
        if score.get("standing_gesture_false_positive"):
            pools["standing_gesture_false_positive"].append(
                _candidate(
                    "standing_gesture_false_positive",
                    wid,
                    None,
                    max(float(score.get("final_semantic_cowgirl_score_v8") or 0.0), 0.30),
                    ["standing/hand/head motion retained as a false-positive audit case"],
                    ["standing_gesture_motion", "hand_head_motion", "not_generation_candidate"],
                )
            )
        if score.get("receiver_response_negative"):
            pools["receiver_response_negative"].append(
        ...
        if score.get("cowgirl_context_intro_low_motion"):
            pools["cowgirl_context_intro_low_motion"].append(
                _candidate(
                    ...
                    None,
                    float(score.get("cowgirl_context_score") or 0.0),
                    ["Cowgirl context/intro or low-motion pose; not clean generation motion"],
                    ["cowgirl_context_intro_low_motion", "not_clean_motion"],
                )
            )
        if score.get("unknown_or_unusable") or score.get("export_unavailable_for_generation"):
            pools["unknown_or_unusable"].append(
                _candidate(
                    "unknown_or_unusable",
                    wid,
                    None,
                    max(float(score.get("final_semantic_cowgirl_score_v8") or 0.0), 0.25),
                    ["unknown/unusable or export unavailable case"],
                    ["unknown_or_unusable"],
                )
            )
    # If there are not enough explicit orientation-invalid examples, let
    # controller-invalid/outlier Cowgirl examples fill that audit slot.
    if len(pools["semantic_cowgirl_orientation_invalid"]) < quotas["semantic_cowgirl_orientation_invalid"]:
        for wid, score in data["cowgirl_scores_v8"].items():
            if score.get("semantic_cowgirl_controller_outlier") and not score.get("semantic_cowgirl_anchor_incomplete"):
                pools["semantic_cowgirl_orientation_invalid"].append(
                    _candidate(
                        "semantic_cowgirl_orientation_invalid",
                        wid,
                        _pair_id_from_role_score(data["rider_receiver_scores"].get(wid, {})),
                        float(score.get("final_semantic_cowgirl_score_v8") or 0.0),
                        ["controller-invalid Cowgirl example used as orientation/controller safety fallback"],
                        ["controller_outlier", "generation_pose_invalid"],
                    )
                )
    for rows in pools.values():
...
def _select_10_candidate_db(data: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    use_v3 = bool({"semantic_cowgirl_core_soft_fail_generation_safe", "not_cowgirl_bj_oral", "semantic_cowgirl_core_hard_fail"} & categories)
    ...
    for wid, record in data.get("candidate_db", {}).items():
        ...
        if use_v3:
            ...
            elif category == "semantic_cowgirl_core_soft_fail_generation_safe":
                pools["soft"].append(candidate)
            elif category == "not_cowgirl_bj_oral":
                pools["bj_oral"].append(candidate)
            elif category == "standing_hand_head_negative":
                pools["standing"].append(candidate)
            elif category in {"receiver_response_negative", "unknown_or_unusable"}:
                pools["receiver_or_unknown"].append(candidate)
            elif category in {"cowgirl_context_intro_low_motion", "export_unavailable_or_unsafe"}:
                pools["context_or_export"].append(candidate)
            elif category in {"semantic_cowgirl_core_hard_fail", "semantic_cowgirl_pose_invalid", "semantic_cowgirl_distance_invalid", "semantic_cowgirl_anchor_incomplete", "semantic_cowgirl_orientation_invalid"}:
                pools["receiver_or_unknown"].append(candidate)
            else:
                pools["receiver_or_unknown"].append(candidate)
        elif use_v2:
...
def _make_review_row(idx: int, item: dict[str, Any], data: dict[str, Any]) -> dict[str, Any]:
    ...
    pose_anchor = data["pose_anchor_completeness"].get(wid, {})
...
def _semantic_guess(
    ...
    if cowgirl_score_v7.get("missing_foot_controllers"):
        movement_labels = _dedupe([*movement_labels, "missing_foot_controllers"])[:5]
    if cowgirl_score_v8.get("controller_rotation_invalid") or orientation_validity.get("controller_rotation_invalid"):
    ...
    return {
        ...
        "semantic_cowgirl_score_v7": cowgirl_score_v7.get("final_semantic_cowgirl_score_v7"),
        "generation_candidate_score_v7": cowgirl_score_v7.get("final_generation_candidate_score_v7"),
        "clean_motion_score_v7": cowgirl_score_v7.get("final_clean_motion_score_v7"),
        "semantic_cowgirl_score_v8": cowgirl_score_v8.get("final_semantic_cowgirl_score_v8"),
        ...
        "semantic_family": candidate_db_record.get("semantic_family"),
        "excluded_from_cowgirl": bool(candidate_db_record.get("excluded_from_cowgirl")),
        "preserve_for_future_dataset": bool(candidate_db_record.get("preserve_for_future_dataset")),
        "core_gate_status": candidate_db_record.get("core_gate_status"),
        "core_gate_can_be_overridden": bool(candidate_db_record.get("core_gate_can_be_overridden")),
        "core_gate_override_reason": candidate_db_record.get("core_gate_override_reason"),
        "bj_oral_confidence": candidate_db_record.get("bj_oral_confidence"),
        "bj_oral_motion_candidate": bool(candidate_db_record.get("bj_oral_motion_candidate")),
        ...
        "missing_foot_controllers": bool(cowgirl_score_v7.get("missing_foot_controllers") or pose_anchor.get("missing_foot_controllers")),
        "missing_knee_controllers": bool(cowgirl_score_v7.get("missing_knee_controllers") or pose_anchor.get("missing_knee_controllers")),
        ...
        "foot_rotation_outlier": bool(cowgirl_score_v8.get("foot_rotation_outlier") or orientation_validity.get("foot_rotation_outlier") or controller_validity.get("foot_rotation_outlier")),
...
def _attempt_timeline_export(row: dict[str, Any], data: dict[str, Any], out_dir: Path, export_mode: str = "motion_only") -> dict[str, Any]:
    ...
    positions, rotations, names, safety = _filter_safe_export_controllers(positions, rotations, names, export_mode=export_mode)
    if not names:
    ...
    meta = {
        ...
        "static_anchor_tracks_included": safety["static_anchor_tracks_included"],
        "static_anchor_controller_names": safety["static_anchor_controller_names"],
        "missing_required_anchor_controllers": row.get("system_semantic_guess", {}).get("missing_required_anchor_controllers", []),
        "pose_anchor_completeness_score": row.get("system_semantic_guess", {}).get("pose_anchor_completeness_score"),
        "generation_pose_anchor_safe": row.get("system_semantic_guess", {}).get("generation_pose_anchor_safe"),
        ...
        "safe_for_import_review": safety["safe_for_import_review"],
        ...
        "orientation_validity_status": row.get("system_semantic_guess", {}).get("orientation_validity_status"),
        "orientation_validity_score": row.get("system_semantic_guess", {}).get("orientation_validity_score"),
        "controller_rotation_invalid": row.get("system_semantic_guess", {}).get("controller_rotation_invalid"),
...
def _filter_safe_export_controllers(positions: np.ndarray, rotations: np.ndarray, names: list[str], export_mode: str = "motion_only") -> tuple[np.ndarray, np.ndarray, list[str], dict[str, Any]]:
    ...
    moving = 0
    anchor_names = [name for name in safe_names if _is_pose_anchor_controller(name)]
    ...
    safety = {
        ...
        "export_mode": export_mode,
        "static_anchor_tracks_included": export_mode in {"motion_plus_static_anchors", "generation_template_candidate"} and bool(anchor_names),
        "static_anchor_controller_names": anchor_names if export_mode in {"motion_plus_static_anchors", "generation_template_candidate"} else [],
        ...
        "safe_for_import_review": "conditional" if bool(safe_names) and teleport_risk in {"low", "medium"} else False,
        ...
        "review_export_warning": "This export is for VaM review only. It preserves source controller coordinates and is not a generative reuse template. Static anchors are included when available in motion_plus_static_anchors mode.",
...
def _is_pose_anchor_controller(name: str) -> bool:
    text = str(name).lower()
    return any(token in text for token in ["footcontrol", "kneecontrol", "thighcontrol", "hipcontrol", "pelviscontrol", "chestcontrol", "abdomencontrol"])
...
def _generation_template_block_reasons(row: dict[str, Any], safety: dict[str, Any]) -> list[str]:
    ...
    if guess.get("controller_rotation_invalid"):
        reasons.append("controller_rotation_invalid")
    if guess.get("controller_twist_invalid"):
        reasons.append("controller_twist_invalid")
    if guess.get("orientation_validity_status") in {"invalid", "unknown"}:
    ...
    if guess.get("generation_pose_anchor_safe") is False:
        reasons.append("pose_anchor_incomplete")
...
def _write_index_html(rows: list[dict[str, Any]], out: Path) -> None:
    ...
    for row in rows:
        ...
        cards.append(
            ...
            f"<li>Semantic Cowgirl score v7: {html.escape(str(guess.get('semantic_cowgirl_score_v7')))}</li>"
            f"<li>Generation candidate score v7: {html.escape(str(guess.get('generation_candidate_score_v7')))}</li>"
            ...
            f"<li>Semantic family: {html.escape(str(guess.get('semantic_family')))} "
            f"(excluded_from_cowgirl={html.escape(str(guess.get('excluded_from_cowgirl')))}, "
            f"preserve_for_future_dataset={html.escape(str(guess.get('preserve_for_future_dataset')))}, "
            f"bj_oral_confidence={html.escape(str(guess.get('bj_oral_confidence')))})</li>"
            f"<li>Core/BJ gates: core={html.escape(str(guess.get('core_controller_gate')))}, "
            f"core_status={html.escape(str(guess.get('core_gate_status')))}, "
            f"override={html.escape(str(guess.get('core_gate_can_be_overridden')))}, "
            f"override_reason={html.escape(str(guess.get('core_gate_override_reason')))}, "
            f"missing_core={html.escape(str(guess.get('missing_core_controllers')))}, "
            f"bj_oral_candidate={html.escape(str(guess.get('bj_oral_motion_candidate')))}, "
