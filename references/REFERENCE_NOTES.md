...
Use the path configured by `VAM_TIMELINE_AI_MOCAP_COMPILER` as the technical reference for scene parsing, Timeline v283 decoding, baking, quaternion handling, validation, preview generation, and offline Timeline export.
...
Use the path configured by `VAM_TIMELINE_AI_TIMELINE_REPO` as the source of truth for AcidBubbles Timeline serialization, compressed keyframe decoding, curve evaluation behavior, and controller target semantics.
...
Use the path configured by `VAM_TIMELINE_AI_VIRTUAL_COMPANION` only as a later playback architecture reference.
...
Use the path configured by `VAM_TIMELINE_AI_RAW_SCENES` as the raw scene reference folder. The first scan pass should be lightweight: list JSON files, identify likely VaM scenes, count Person atoms, and detect technical motion carriers. It should not bake, train, export, or deeply label semantics.
